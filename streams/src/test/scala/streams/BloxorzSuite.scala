...
class BloxorzSuite {
  ...
  @Test def `neighbors with history`: Unit =
    new Level1 {
      val block: Block = Block(Pos(1, 1), Pos(1, 1))
      val moveHistory = List(Left, Up)

      val expectedList = LazyList(
        (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
        (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
      )
      assertEquals(expectedList, neighborsWithHistory(block, moveHistory))
    }
  ...
  @Test def `new neighbors only`: Unit =
    new Level1 {
      ...
      val expectedList = LazyList((Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up)))
      assertEquals(expectedList, newNeighborsOnly(neighbors, exploredSet))
    }
  ...
  @Test def `impossible level, solution should be empty`: Unit =
    new ImpossibleLevel {
