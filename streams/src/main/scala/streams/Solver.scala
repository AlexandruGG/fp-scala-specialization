...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    for {
      neighbors <- b.legalNeighbors.to(LazyList)
    } yield (neighbors._1, neighbors._2 :: history)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] =
    neighbors.filter((n: (Block, List[Move])) => !explored.contains(n._1))
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] =
    if (initial.isEmpty) LazyList.empty
      ...
      val nextList = for {
        initialList <- initial
        nextList <- newNeighborsOnly(neighborsWithHistory(initialList._1, initialList._2), explored)
      } yield nextList

      initial ++ from(nextList, explored ++ nextList.map(_._1))
    }
