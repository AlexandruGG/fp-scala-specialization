...
class CodecsSuite
  ...
  @Test def `an object value should be encoded as a JSON Object (0pts)`(): Unit = {
    val xs = 1 :: 2 :: Nil
  ...
  @Test def `an empty JSON array should be decoded as empty list`(): Unit = {
    val emptyJsonArray = Json.Arr(List())
    val expected = Some(List())

    val decoder = implicitly[Decoder[List[Int]]]
    assert(decoder.decode(emptyJsonArray) == expected)
  }
