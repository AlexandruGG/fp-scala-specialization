...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction { case Json.Arr(items) => items.flatMap(decoder.decode) }
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction { case Json.Obj(fields) if fields.contains(name) => decoder.decode(fields(name)) }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person](fields => Person(fields._1, fields._2))
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] =
    ObjectEncoder.field[String]("people")
      .zip(ObjectEncoder.field[List[Person]]("people"))
      .transform[Contacts](contacts => ("people", contacts.people))
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val maybeJsonObj3 = parseJson(""" { "people": [{ "name": "Alice", "age": 42 }, { "name": "Bob", "age": 33 }] } """)
    ...
    println(renderJson(Contacts(List(Person("Bob", 66)))))
  }
