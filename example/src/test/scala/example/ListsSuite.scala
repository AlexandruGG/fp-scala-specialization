...
class ListsSuite {
  ...
  @Test def `sum with negative numbers`: Unit = {
    assertEquals(-8, sum(List(-2, -1, -5)))
  }

  @Test def `sum with negative and positive numbers`: Unit = {
    assertEquals(2, sum(List(-2, -1, 5)))
  }

  @Test def `sum with all zeros`: Unit = {
    assertEquals(0, sum(List(0, 0, 0)))
  }

  @Test def `sum with empty list`: Unit = {
    assertEquals(0, sum(List()))
  }

  @Test def `max of empty list throw exception`: Unit = {
    try {
      max(List())
      Assert.fail("No exception has been thrown")
    } catch {
  ...
  @Test def `max with negative and positive numbers numbers`: Unit = {
    assertEquals(5, max(List(-2, -1, 5)))
  }

  @Test def `max with all zeros`: Unit = {
    assertEquals(0, max(List(0, 0, 0)))
  }

  @Test def `max with repeating elements`: Unit = {
    assertEquals(2, max(List(1, 2, 2)))
  }
