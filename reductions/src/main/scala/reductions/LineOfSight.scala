...
object LineOfSight extends LineOfSightInterface {
  private def angle(a: Float, b: Int): Float = a / b

  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    (1 until input.length).foreach(i => output(i) = Math.max(output(i - 1), angle(input(i), i)))
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    (from until until).foldLeft(Float.MinValue)((currentMax, i) => List(angle(input(i), i), currentMax).max)
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    var current = startingAngle
    (from until until).foreach { i =>
      current = Math.max(current, angle(input(i), i))
      output(i) = current
    }
  }
